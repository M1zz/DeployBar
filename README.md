...
- **스크린샷이 낡으면 체크리스트가 먼저 말한다.** 그림을 찍은 뒤로 화면 파일이 바뀌었으면
  ...
  **배포는 막지 않는다**(⚠️ 권장). `docs/screenshots/` 가 없는 앱에는 아예 뜨지 않는다 —
  이 워크플로를 안 쓰는 앱에 고칠 생각 없는 경고를 붙이지 않기 위해서다.
- **`⋯ › 앱스토어 그림 프롬프트`** = (낡지 않았어도) 언제든 같은 지시문을 꺼낸다.
  DeployBar 는 그림을 만들지도 올리지도 않는다. 대신 **무엇을 다시 찍어야 하는지**는 이쪽이 안다 —
...
- App Store 심사 자동제출은 미포함.
- **스크린샷·미리보기 영상은 만들지도 올리지도 않는다.** 지시문만 만들어 준다
  (`⋯ › 앱스토어 그림 프롬프트` · `--shots`). 찍는 건 Claude Code, 스토어에 올리는 건 사람이 웹에서.
